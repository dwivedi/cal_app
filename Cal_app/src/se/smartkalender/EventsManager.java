...
public class EventsManager {	
	...
	static ArrayList<SmartCalendarEvent>  events = new ArrayList<SmartCalendarEvent>(0);
	...
	static String[] projection = { EventsContentProvider.EVENT_ID, 	
			EventsContentProvider.TITLE, 
			EventsContentProvider.DESCRIPTION,
			EventsContentProvider.ICON_ID,
			EventsContentProvider.COLOR,
			EventsContentProvider.DATE,
			EventsContentProvider.START_TIME,
			EventsContentProvider.FINISH_TIME,
			EventsContentProvider.REMINDER,
	...
	public static ArrayList<SmartCalendarEvent> getAllEvents(Date selectedDay) {
		ArrayList<SmartCalendarEvent> _events = new ArrayList<SmartCalendarEvent>(0);
	...
	public static  ArrayList<SmartCalendarEvent> getEventsForPeriod(Date startTime, int periodMinutes) {
		ArrayList<SmartCalendarEvent> _events = new ArrayList<SmartCalendarEvent>(0);
		for (SmartCalendarEvent event : events) {
			if (!isEventOnDate(event, startTime)) continue;				
			if (globals.isTimeAfter(event.getStartTime(), globals.getTimePlusSeconds(startTime, 0))) {
				if (globals.isTimeBefore(event.getStartTime(), globals.getTimePlusSeconds(startTime, periodMinutes*60))) {
					_events.add(event);	
				}				
			}					
		}
	...
	public static boolean isEventOnDate(SmartCalendarEvent event, Date startTime) {
		if (globals.isEqualDate(event.getDate(), startTime)) return true;
		if (event.getDate().after(startTime)) return false;
		...
		if (repeat_days.length> 0) {	
			...
			for (int i = 0; i < repeat_days.length; i++) {
				if (globals.weekDays[dayOfWeek].contentEquals(repeat_days[i])) return true;				
			}					
		}
		return false;
	}
	...
	public static int changeEvent(int id, SmartCalendarEvent event, Activity a) {
		...
		// if no event with id and id == 0, then it appears that user add new event
    ...
    public static ArrayList<SmartCalendarEvent> loadEventsFromTemplateFileConfig(Context context) {        	
        InputStream inputStream = context.getResources().openRawResource(R.raw.events_templates);
        templateEvents = new ArrayList<SmartCalendarEvent>(0);
        try {
    ...
    private static ArrayList<String> extractTemplateCategories(ArrayList<SmartCalendarEvent> templates) {
        ...
        for( SmartCalendarEvent template : templates ){
        	globals.addRecordToLog("checking " + template.getName() + " (" + template.getCategory() + ")");

        	globals.addRecordToLog("compare category "+template.getCategory()+" with "+lastCategory + " results in "+lastCategory.compareToIgnoreCase(template.getCategory()));
        
        	if( lastCategory.compareToIgnoreCase(template.getCategory())!=0){
        		globals.addRecordToLog("found category "+template.getCategory());
	...
	public static int getEventPeriod(SmartCalendarEvent event) {		
		return globals.getDifferenceInMinutes(event.getFinishTime(), event.getStartTime());
	}
	...
	private static void addEventToDB(SmartCalendarEvent event, Activity a){
		...
		values.put(EventsContentProvider.START_TIME, event.getStartTime().getTime());
		values.put(EventsContentProvider.FINISH_TIME, event.getFinishTime().getTime());
	...
	private static void removeEventFromDB(Activity a, SmartCalendarEvent event){	
		ContentResolver cr = a.getContentResolver(); 
		cr.delete(EventsContentProvider.CONTENT_URI, "_id=?", new String[] {String.valueOf(event.getId())});
	}
	
	public static void extractEventsFromDB(Context context){		
		events.clear();
		try {
		Cursor cursor = context.getContentResolver().query(EventsContentProvider.CONTENT_URI, projection, null, null,	null);
		if (cursor != null) {
			cursor.moveToFirst();		
			while (true) {	
				int id = cursor.getInt(cursor.getColumnIndexOrThrow(EventsContentProvider.EVENT_ID));
				String iconId = cursor.getString(cursor.getColumnIndexOrThrow(EventsContentProvider.ICON_ID));
				String desc = cursor.getString(cursor.getColumnIndexOrThrow(EventsContentProvider.DESCRIPTION));
				String name = cursor.getString(cursor.getColumnIndexOrThrow(EventsContentProvider.TITLE));
				int color = cursor.getInt(cursor.getColumnIndexOrThrow(EventsContentProvider.COLOR));
				Date date = new Date();
				date.setTime(cursor.getLong(cursor.getColumnIndexOrThrow(EventsContentProvider.DATE)));
				Date startTime = new Date();
				startTime.setTime(cursor.getLong(cursor.getColumnIndexOrThrow(EventsContentProvider.START_TIME)));
				Date finishTime = new Date();
				finishTime.setTime(cursor.getLong(cursor.getColumnIndexOrThrow(EventsContentProvider.FINISH_TIME)));
				String repeatDays = cursor.getString(cursor.getColumnIndexOrThrow(EventsContentProvider.REPEAT_DAYS));
				Integer reminderTime = cursor.getInt(cursor.getColumnIndexOrThrow(EventsContentProvider.REMINDER));
				
				SmartCalendarEvent e = new SmartCalendarEvent(id, name, iconId, color, date, startTime, finishTime);
	...
	public static EventAlarmTime getNextEventAlarmTime() {
		...
		globals.addRecordToLog("getNextEventAlarmTime - " + Integer.toString(events.size()));
		for (SmartCalendarEvent event : events) {
			...
			globals.addRecordToLog("getNextEventAlarmTime going to compare - " + 
					DateFormat.format("EEEE d MMM, kk:mm", nextTimeForEvent) + " with " + DateFormat.format("EEEE d MMM, kk:mm", currentTime));
			if (nextTimeForEvent != null && nextTimeForEvent.after(currentTime)) {
				if (timeToReturn == null || timeToReturn.after(nextTimeForEvent)) {
		...
		if (timeToReturn != null) {
			globals.addRecordToLog("getNextEventAlarmTime timeToReturn = " + DateFormat.format("EEEE d MMM, kk:mm", timeToReturn));
			EventAlarmTime eat = new EventAlarmTime(returnEvent, timeToReturn.getTime());
			return eat;
		}
		else return null;
	}
	...
	public static Date getNextTimeForEvent(SmartCalendarEvent event, Date currentTime, boolean remind) {
		ArrayList<String> repeatDays = new ArrayList<String>(Arrays.asList(event.getRepeatDays().split(",")));
		Date date = new Date();
		...
		if (date.before(currentTime)) {
			if (repeatDays.size() == 0) return null;
			...
			date = globals.getBeginOfDayPlusSeconds(currentTime, (int)(daysAfterToday * globals.SECONDS_PER_DAY));
		}
		// if reminder exist using it first
		Date startTime = new Date();
		if (remind) {
			Date reminderTime = event.getReminderTime();
			if (reminderTime == null) startTime = event.getStartTime();
			else {
				...
				if (globals.isTimeAfter(currentTime, startTime)) startTime = event.getStartTime();
			}
		}
		else startTime = event.getStartTime();
	...
	public static void setAlarmForFirstEvent(Context context, boolean snooze) {
		...
		if (eventAlarmTime != null) i.putExtra("eventId", eventAlarmTime.event.getId());			
		...
		if (eventAlarmTime != null) {
			if (snooze) {
				Date currentTime = new Date();
				currentTime.setTime(currentTime.getTime() + globals.MILLISECONDS_IN_5_MINUTES);
				if (currentTime.getTime() < eventAlarmTime.nextTimeForEvent) {
					globals.addRecordToLog("setting reminder to " + DateFormat.format("EEEE d MMM, kk:mm", currentTime).toString());
					am.set(AlarmManager.RTC_WAKEUP, currentTime.getTime(), pendingIntent);	
					return; 
				}
			}
			globals.addRecordToLog("setting reminder to " +  DateFormat.format("EEEE d MMM, kk:mm", eventAlarmTime.getDate()));
			am.set(AlarmManager.RTC_WAKEUP, eventAlarmTime.nextTimeForEvent, pendingIntent);
		}
		else globals.addRecordToLog("setAlarmForFirstEvent eventAlarmTime = null");
	}
