...
public class EventNotificationDialog extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		SmartCalendarEvent event = new SmartCalendarEvent(EventsManager.getEvent(getIntent().getExtras().getInt("eventId")));
		...
		if (event.getStartTime().before(currentTime)) message.setText(globals.TheEventWillStartNow);
		else {
			long minutesBeforeEvent = (event.getStartTime().getTime() - currentTime.getTime()) / globals.MILLISECONDS_IN_1_MINUTES;
			message.setText(globals.TheEventWillStartIn + " " +  Long.toString(minutesBeforeEvent)  + " " + globals.minutes);
		}		
		// use PowerManager to on screen and KeyguardManager to disable keyguard
		PowerManager pm = (PowerManager) getSystemService(POWER_SERVICE);
		wl = pm.newWakeLock(PowerManager.ACQUIRE_CAUSES_WAKEUP | PowerManager.FULL_WAKE_LOCK, "smartcalendar");
		wl.acquire();		
		/*KeyguardManager km = (KeyguardManager) getSystemService(KEYGUARD_SERVICE);
		kl = km.newKeyguardLock("smartcalendar");
		kl.disableKeyguard();	*/	
		...
		btnGoToCalendar.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
            	Intent i = new Intent(EventNotificationDialog.this, SmartCalendarActivity.class);
...
/*		
		...
		btnShowDetails.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                Intent intent = new Intent(EventNotificationDialog.this, EventDetailsActivity.class);
...
*/		
		...
		btnSnooze.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
            	...
            	if (wl != null && wl.isHeld()) wl.release();
            	if (mediaPlayer != null) mediaPlayer.stop();
            	EventsManager.setAlarmForFirstEvent(EventNotificationDialog.this, true);	
                finish();
            }
        });
		try {
	...
	void playSound() throws IllegalArgumentException, SecurityException, IllegalStateException, IOException {
